...
c-send は、AIや外部サービスの設定なしで単体利用できる、Windows用の定型文・コマンド管理ツールです。よく使う文章やコマンドをカテゴリごとに整理し、必要な内容をすぐクリップボードへ送れます。

インストール後に`csend.exe`を起動してカテゴリを選ぶだけで、定型文・コマンド・テンプレートを利用できます。AIアカウントやAPIキーは必要ありません。

v2.0では、必要な人向けのオプション機能として、定型文を外部APIへ送信して結果を受け取るAPI機能も追加しました。Gemini、OpenAI、Anthropic Claude API、GitHub REST API、Open-Meteoなどを、設定JSONで利用できます。
...
理念、AIとの責任分担、AIに設定を作らせる手順、個人コンテキスト、順序付きワークフロー、将来の企業向け拡張は [AI連携方針・拡張設計](docs/ai-integration.md) にまとめています。
